...
def paper_cl_metrics_zheng2026(acc_table, task_num, A_ref=None, eps=1e-8):
    """
    Continual learning metrics from Zheng et al., ICLR 2026 (arXiv:2602.17559), Section 4.1.

    Uses the same notation as the paper: A_{t,i} is accuracy on task i after training on task t
    (1-indexed in the paper). Here acc_table[k, j] is the accuracy on task j after training task k
    (0-indexed), for j <= k, matching LibContinual's acc_table.

    - A_t (paper): average accuracy after training task t, A_t = (1/t) * sum_{i=1}^t A_{t,i}.
    - Avg.: overall average (1/T) * sum_t A_t. For T=10 this matches the paper's "Avg." column.
    - A_T: average accuracy after the last task, i.e. A_t at t=T; matches the paper's "A10" style
      column when T=10.
    - F: normalized forgetting (Eq. 5); Stability = 1 - F.
    - Plasticity (Eq. 6): (1/T) * sum_i A_{i,i} / A^{ref}_{i,i}, same units as acc_table (e.g. %).
      Supply A_ref as a length-T list of reference accuracies (isolated fine-tune per task), or leave
      None to skip.
    """
    T = int(task_num)
    acc_table = np.asarray(acc_table, dtype=np.float64)
    A_t = []
    for k in range(T):
        A_t.append(float(np.mean(acc_table[k, : k + 1])))
    avg_paper = float(np.mean(A_t)) if T > 0 else 0.0
    A_final = A_t[-1] if T > 0 else 0.0
    ...
    if T < 2:
        ...
        terms = []
        for j in range(T - 1):
            peak = max(float(acc_table[t, j]) for t in range(j, T - 1))
            final = float(acc_table[T - 1, j])
            denom = max(peak, eps)
            terms.append((peak - final) / denom)
        F = float(np.mean(terms))

    stability = 1.0 - F

    plasticity = None
    plasticity_ratios = None
    A_ref_arr = None
    A_ii = None
    if T > 0:
        A_ii = np.array([float(acc_table[i, i]) for i in range(T)], dtype=np.float64)
    if A_ref is not None and T > 0:
        A_ref_arr = np.asarray(A_ref, dtype=np.float64).reshape(-1)
        if A_ref_arr.shape[0] != T:
            raise ValueError(
                f"plasticity_reference_acc length {A_ref_arr.shape[0]} must equal task_num {T}"
            )
        plasticity_ratios = A_ii / np.maximum(A_ref_arr, eps)
        plasticity = float(np.mean(plasticity_ratios))
    ...
    return {
        ...
        "F_normalized": F,
        "Stability": stability,
        "A_ii": None if A_ii is None else A_ii.tolist(),
        "A_ref": None if A_ref_arr is None else A_ref_arr.tolist(),
        "Plasticity": plasticity,
        "plasticity_ratios": None if plasticity_ratios is None else plasticity_ratios.tolist(),
    }
