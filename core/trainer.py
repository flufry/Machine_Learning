...
class Trainer(object):
    ...
    def train_loop(self,):
        ...
        if self.rank == 0:
            ...
            show_paper_metrics = (
                method_name == "EWCLoRA"
                or self.config.get("log_paper_cl_metrics", False)
                or self.config.get("plasticity_reference_acc") is not None
                or (
                    self.config.get("compute_plasticity_reference", False)
                    and method_name == "EWCLoRA"
                )
            )
            if show_paper_metrics:
                A_ref = self.config.get("plasticity_reference_acc")
                if A_ref is not None:
                    A_ref = [float(x) for x in A_ref]
                elif self.config.get("compute_plasticity_reference", False) and method_name == "EWCLoRA":
                    print(
                        "================Plasticity references (isolated fine-tune per task)================"
                    )
                    print(
                        " * Running T single-task runs (ViT + LoRA, no EWC); this matches Eq. 6 A_ref in the paper."
                    )
                    from core.model.ewc_lora import run_reference_single_task_finetunes_ewclora

                    A_ref = run_reference_single_task_finetunes_ewclora(
                        self.config,
                        self.device,
                        self.task_num,
                        self.init_cls_num,
                    ...
                    print(f" * Computed A_ref (%, per task): {[round(x, 2) for x in A_ref]}")
                else:
                    A_ref = None

                pm = paper_cl_metrics_zheng2026(acc_table, self.task_num, A_ref=A_ref)
                print(f"================Paper metrics (ICLR 2026 Sec 4.1, Zheng et al.)================")
                print(f" * A_T (last-step avg over all tasks, e.g. A10 when T=10): {pm['A_T']:.2f}")
                print(f" * Avg. (mean over t of A_t): {pm['Avg_paper']:.2f}")
                print(f" * A_t curve: {[round(x, 2) for x in pm['A_t']]}")
                print(f" * Normalized forgetting F: {pm['F_normalized']:.4f}")
                print(f" * Stability (1 - F): {pm['Stability']:.4f}")
                if pm["Plasticity"] is not None:
                    print(f" * Plasticity (Eq. 6): {pm['Plasticity']:.4f}")
                    print(f" * A_i,i (diag, %): {[round(x, 2) for x in pm['A_ii']]}")
                    print(f" * A_ref (%, per task): {[round(x, 2) for x in pm['A_ref']]}")
                    print(
                        f" * Ratios A_i,i/A_ref: {[round(x, 4) for x in pm['plasticity_ratios']]}"
                    )
                else:
                    print(
                        " * Plasticity (Eq. 6): (skipped — set plasticity_reference_acc in yaml, or "
                        "compute_plasticity_reference: true with EWCLoRA)"
                    )
