...
def _binary_cifar100_pickle_dir(data_root):
    """Parent dir of official `train` / `test` pickle files (CIFAR-100 Python/Matlab layout)."""
    nested = os.path.join(data_root, "cifar-100-python")
    if os.path.isfile(os.path.join(nested, "train")):
        return nested
    if os.path.isfile(os.path.join(data_root, "train")):
        return data_root
    return nested
...
class ContinualDatasets:
    def __init__(self, dataset, mode, task_num, init_cls_num, inc_cls_num, data_root, cls_map, trfms, batchsize, num_workers, config):
        ...
        if self.dataset == "binary_cifar100":
            pickle_dir = _binary_cifar100_pickle_dir(self.data_root)
            if not os.path.isfile(os.path.join(pickle_dir, "train")):
...
class SingleDataset(Dataset):
    ...
    def _init_datalist(self):
        ...
        if self.dataset == 'binary_cifar100':
            pickle_dir = _binary_cifar100_pickle_dir(self.data_root)
            with open(os.path.join(pickle_dir, self.mode), "rb") as f:
